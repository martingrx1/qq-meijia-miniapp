...
import { parseDateToTimestamp, parseTimestampToDate, isMonday, findNearMonday } from '../../utils/date';
...
Page({
  ...
  data: {
    ...
    morningSubTime: ['9:00-11:00'],
    afternoonSubTime: ['2:30-4:30', '15:00-17:30'],
    eveningSubTime: ['17:00-19:00', '19:00-21:00'],
    dateIndex: 0, //当前选择日期数
    lazyDog: false, //懒狗模式
    weekPlan: [],
    timeQuantum: [],
  ...
  pickerWeek(status, value) {
    ...
    if (!isMonday(value)) {
      //星期一设置错误
      wx.showToast({ title: '必须选择周一', icon: 'none' });
      return;
    }

    wx.showLoading({ title: `正在加载日期配置` });

    this.selectQueryMethodsForStamp(stampValue); //加载选择的周数据
  ...
  setDayPlan() {
    //当前页面信息切换为当日配置
    const D = this.data;
    let i = D.dateIndex; // 默认 0 星期一
    //morningCapacity等 统一使用dayplan里面的数据
    this.setData({
      ...
      morningCapacity: D.timeQuantum[i].dayPlan[0].capacity,
      afternoonCapacity: D.timeQuantum[i].dayPlan[1].capacity,
      eveningCapacity: D.timeQuantum[i].dayPlan[2].capacity
    });
  },

  setWeekPlan() {
    //将修改后的当日配置信息覆盖至周配置信息中
    let data = this.data;
  ...
  initTemplateData(startDateTimestamp = new Date().getTime()) {
    //页面配置为从本地模板新增
  ...
  initSubTime(startDateTimestamp) {
    const { previousDate, previousTimestamp, nextDate, nextTimestamp } = findNearMonday(startDateTimestamp);
    this.setData({
